...
//    lookated = glm::normalize(lookated);
    ...
    glm::vec4 tmp;
    glm::mat4 rot = glm::rotate(glm::mat4(1.0f), updateAngle.y, axis);
    tmp = rot * glm::vec4(pos, 1.0f);
    tmp = tmp / tmp.w;
    pos = glm::vec3(tmp);
    pos = pos + rotateCenterPoint;
    tmp = rot * glm::vec4(lookated, 1.0f);
    tmp = tmp / tmp.w;
    lookated = (glm::vec3(tmp) + rotateCenterPoint) - pos;
...
bool viewer::pointInFrustum(glm::vec3 pt) {
    ...
    glm::vec4 pt_on_scr = pvm * glm::vec4(pt, 1.0f);
    pt_on_scr = pt_on_scr / pt_on_scr.w;
    if ((pt_on_scr.z >= 0.0f) && gl3d::math::point_in_range(glm::vec2(pt_on_scr.x, pt_on_scr.y), glm::vec2(-1.0f), glm::vec2(1.0f)))
        return true;
    else
        return false;
